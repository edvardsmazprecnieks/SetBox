...
def schedule(date_string):
    ...
    for date in weekdates:
        ...
        for lesson in lessons:
            if lesson.Lesson.start_time < min_time:
                min_time = lesson.Lesson.start_time
            if lesson.Lesson.end_time > max_time:
                max_time = lesson.Lesson.end_time
    min_time_rounded = min_time.replace(microsecond=0, second=0, minute=0)
    if max_time.replace(hour=0) > time(minute=0):
        max_time_rounded = max_time.replace(
            microsecond=0, second=0, minute=0, hour=max_time.hour + 1
        )
    else:
        max_time_rounded = max_time.replace(microsecond=0, second=0)
    if min_time_rounded > max_time_rounded:
        return render_template(
            "schedule/schedule.html", weekdates=weekdates, times=[], lessons=[]
        )
    ...
    return render_template(
        "schedule/schedule.html", weekdates=weekdates, times=times, lessons=lessons_list
    )
