...
def schedule(date_string):
    ...
    for date in weekdates:
        ...
        lessons = db.session.query(Lesson, Subject).filter(Lesson.formatted_date == date).filter(Lesson.subject_id == Subject.id).filter(Subject.owner_user_id == current_user.id).all()
