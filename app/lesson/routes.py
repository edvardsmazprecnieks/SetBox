...
@login_required
def addlesson():
    subjects = Subject.query.filter(Subject.id == UserInSubject.subject_id).filter((Subject.owner_user_id == current_user.id)|(UserInSubject.user_id == current_user.id)).filter(UserInSubject.editor==True).all()
